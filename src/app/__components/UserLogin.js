...
export default function UserLogin({ setOption , USER }) {
  ...
  return (
    <div className='fixed top-0 bottom-0 w-[100vw] min-h-[100vh] flex flex-col gap-4 items-center justify-center'>
      ...
      <form onSubmit={handleLogin} className='bg-blue-50 shadow-[0_0_10px_black] w-[98%] max-w-[500px] py-4 px-5 rounded-2xl min-h-[280px] text-red-950 flex flex-col items-center justify-evenly' autoComplete='on'>
        ...
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className='bg-blue-950/10 px-2 py-[10px] rounded text-center w-[96%] max-w-[550px] outline-none focus:bg-green-600/15 ring-1 focus:ring-2 ring-violet-700' placeholder='email' name='email' required />

        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className='bg-blue-950/10 px-2 py-[10px] rounded text-center w-[96%] max-w-[550px] outline-none focus:bg-green-600/15 ring-1 focus:ring-2 ring-violet-700' placeholder='password' name='password' required />
