...
export async function POST(req) {
    try {
        ...
        const donation = await DonatedUnit.find({ donorId:body.receiverId , status:false });
        if(donation)    return NextResponse.json({ success:false, message:"your have a donation pending as you want to donate blood and you can not be a donor and a receiver at the same time." });
